...
- Скрипт `npm run prisma:seed` создаёт активную учётную запись администратора. По умолчанию это `admin@octava.ru` / `changeme`, но значения можно переопределить через переменные `ADMIN_EMAIL` и `ADMIN_PASSWORD` перед запуском сида.【F:prisma/seed.ts†L17-L34】
