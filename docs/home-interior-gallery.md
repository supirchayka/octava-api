...
## Что отдает API
- `GET /admin/pages/home` возвращает блок `interior` с полями `text` и **полным списком** `images` без ограничений по количеству. Каждый элемент содержит `id`, `fileId`, `url`, `alt`, `caption`, `order` и формируется прямо из записей `HomeGalleryImage` в БД (ни один элемент не берется из сида). Если `alt` не задан, он подставляется из `originalName` файла. 【F:src/services/admin-pages.service.ts†L281-L352】
