...
Ниже перечислены все публичные эндпоинты, их параметры и структура ответов, сформированная текущей реализацией сервисов. Авторизация на пользовательском фронтенде не требуется.
...
### `GET /services/prices/female` и `GET /services/prices/male`
- Формат ответа и поля описаны в `docs/service-prices.md`.
...
1. **Конфигурация API**
   ...
   - Публичный фронтенд работает без авторизации; дополнительные заголовки не требуются.
...
5. **Ошибки и статусы**
   - Обрабатывать 400/404 по тексту сообщения из поля `message`, выводя пользовательскую ошибку.
   - При сетевых ошибках или 5xx показывать общий фолбек.

6. **Кеширование и SEO**
   - Позволяется кешировать публичные GET-ответы на стороне клиента/SSR; формы не кешировать.
