...
class CatalogService {
    ...
    mapSeo(seo) {
        ...
        return {
            ...
            ogImage: seo.ogImage
                ? {
                    url: (0, files_1.buildFileUrl)(seo.ogImage.path),
    ...
    async getServiceBySlug(slug) {
        ...
        const [seo, details, pricesExtended, indications, contraindications, prepSteps, rehabSteps, faq, legalDisclaimer, deviceLinks, images,] = await Promise.all([
        ...
        const heroImages = images
            .filter((img) => img.purpose === client_1.ImagePurpose.HERO)
            .map((img) => ({
            id: img.id,
            url: (0, files_1.buildFileUrl)(img.file.path),
            alt: img.alt ?? img.file.originalName,
            caption: img.caption,
            order: img.order,
        }));
        const galleryImages = images
            .filter((img) => img.purpose === client_1.ImagePurpose.GALLERY)
            .map((img) => ({
            id: img.id,
            url: (0, files_1.buildFileUrl)(img.file.path),
            alt: img.alt ?? img.file.originalName,
        ...
        const inlineImages = images
            .filter((img) => img.purpose === client_1.ImagePurpose.INLINE)
            .map((img) => ({
            id: img.id,
            url: (0, files_1.buildFileUrl)(img.file.path),
            alt: img.alt ?? img.file.originalName,
    ...
    async getDeviceBySlug(slug) {
        ...
        const [seo, certBadges, attachments, indications, contraindications, sideEffects, documents, faq, images, serviceLinks,] = await Promise.all([
        ...
        const certs = certBadges.map((c) => ({
            ...
            image: c.image
                ? {
                    id: c.image.id,
                    url: (0, files_1.buildFileUrl)(c.image.path),
            ...
            file: c.file
                ? {
                    id: c.file.id,
                    url: (0, files_1.buildFileUrl)(c.file.path),
                    mime: c.file.mime,
        ...
        const docs = documents.map((d) => ({
            ...
            file: d.file
                ? {
                    id: d.file.id,
                    url: (0, files_1.buildFileUrl)(d.file.path),
                    mime: d.file.mime,
        ...
        return {
            ...
            attachments: attachments.map((a) => ({
                ...
                image: a.image
                    ? {
                        id: a.image.id,
                        url: (0, files_1.buildFileUrl)(a.image.path),
