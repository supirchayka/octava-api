...
class PagesService {
    ...
    dayGroupLabel(group) {
        switch (group) {
            case client_1.DayGroup.WEEKDAYS:
                return "Пн–Пт";
            case client_1.DayGroup.SATURDAY:
                return "Суббота";
            case client_1.DayGroup.SUNDAY:
                return "Воскресенье";
            default:
                return String(group);
        }
    ...
    async getHome() {
        ...
        const heroImages = page.home.gallery
            .filter((g) => g.purpose === client_1.ImagePurpose.HERO)
            .map((g) => ({
            id: g.id,
            url: (0, files_1.buildFileUrl)(g.file.path),
            alt: g.alt ?? g.file.originalName,
            caption: g.caption,
            order: g.order,
        }));
        const interiorImages = page.home.gallery
            .filter((g) => g.purpose === client_1.ImagePurpose.GALLERY)
            .map((g) => ({
            id: g.id,
            url: (0, files_1.buildFileUrl)(g.file.path),
            alt: g.alt ?? g.file.originalName,
    ...
    async getAbout() {
        ...
        const trust = page.about.trustItems.map((t) => ({
            ...
            image: t.image
                ? {
                    id: t.image.id,
                    url: (0, files_1.buildFileUrl)(t.image.path),
            ...
            file: t.file
                ? {
                    id: t.file.id,
                    url: (0, files_1.buildFileUrl)(t.file.path),
                    mime: t.file.mime,
    ...
    async getOrgInfo() {
        const page = await this.app.prisma.staticPage.findUnique({
            where: { type: client_1.StaticPageType.ORG_INFO },
            include: {
                seo: {
                    include: {
                        ogImage: true,
        ...
        return {
            ...
            licenses: org.licenses.map((lic) => ({
                ...
                file: lic.file
                    ? {
                        id: lic.file.id,
                        url: (0, files_1.buildFileUrl)(lic.file.path),
                        mime: lic.file.mime,
    ...
    async getPersonalDataPolicy() {
        return this.getPolicyByType(client_1.StaticPageType.PERSONAL_DATA_POLICY);
    }
    async getPrivacyPolicy() {
        return this.getPolicyByType(client_1.StaticPageType.PRIVACY_POLICY);
    }
}
