...
- **Формат**: если не указано иное, тело запросов и ответов — JSON. Загрузку файлов выполняем через `multipart/form-data` либо через POST с «сырым» бинарным телом (например, `fetch` + `Blob`). Для второго варианта желательно добавить заголовок `X-Filename`, чтобы сохранить расширение; без него бэкенд определит тип автоматически.【F:src/controllers/admin-files.controller.ts†L22-L73】【F:src/services/admin-files.service.ts†L96-L187】
...
- `multipart/form-data` с полем `file` — стандартный вариант.
- запрос с бинарным телом (Blob/ArrayBuffer). Можно передать любой корректный `Content-Type`. Если заголовка нет, сервер распознает сигнатуру (JPEG/PNG/WEBP/PDF и т.п.) и назначит расширение самостоятельно. Заголовок `X-Filename` позволяет сохранить оригинальное имя файла.
...
Каждый маршрут `/admin/pages/...` работает в формате «GET для чтения, PUT для частичного сохранения». Ниже показаны полные JSON-ответы (все ключи присутствуют, даже если значение `null`).
...
```json
{
  ...
  "subheroImage": {
    "id": 12,
    "path": "/uploads/seed/subhero-home.jpg",
    "mime": "image/jpeg",
    "originalName": "subhero-home.jpg",
    "sizeBytes": 240100
  },
...
**PUT Body** — опциональные `heroTitle`, `heroSubtitle`, `heroCtaText`, `heroCtaUrl`, `subheroTitle`, `subheroSubtitle`, `interiorText`, `seo`, а также массивы `heroImages` (одно изображение HERO), `interiorImages` (галерея) и `directions` (ровно четыре объекта `{ serviceId, order? }`).
...
**PUT Body** — `heroTitle`, `heroDescription`, `howWeAchieveText`, `heroCtaTitle`, `heroCtaSubtitle`, `heroImageFileId`, `facts` (массив `{ title, text, order? }`), `seo`.
...
```json
{
  ...
  "whatsappUrl": null,
  "addressText": "Москва, ул. Пример, 10",
...
**PUT Body** — `phoneMain`, `email`, `telegramUrl`, `whatsappUrl`, `addressText`, `yandexMapUrl`, `workingHours` (массив уникальных `DayGroup` с `open/close` в `HH:MM`), `metroStations` (массив `{ name, distanceMeters?, line? }`), `seo`.
...
**PUT Body** — `title`, `body`, `seo` (можно передавать HTML).
